...
class MyListener(jauanListener):
    ...
    def enterMain(self, ctx: jauanParser.MainContext):
        ...
        if self.searchNameFunction('main') == None:
            ...
            raise SystemExit(Exception("A função main já foi declarada."))
    ...
    def exitMain(self, ctx: jauanParser.MainContext):
        self.jasmin.endMain()
        self.jasmin.exit()
        limit = self.tabelaNameFunctions[self.searchNameFunction(
            self.escopo)][LAST_LOCAL_USED]
        self.jasmin.remakeLimits(limit+50, limit+50)
        self.jasmin.open()
        self.tabelaDeSimbolos = {}
        pass
    ...
    def enterDeclar_funcao(self, ctx: jauanParser.Declar_funcaoContext):
        ...
        if self.searchNameFunction(ctx.ID_L().getText()) == None:
            if len(self.tabelaNameFunctions.keys()) == 0:
                _id = 0
                self.tabelaNameFunctions[_id] = [
                    ctx.ID_L().getText(), ctx.TIPO().getText()]
            else:
                _id = max(self.tabelaNameFunctions.keys()) + 1
                self.tabelaNameFunctions[_id] = [
                    ctx.ID_L().getText(), ctx.TIPO().getText()]
            ...
            raise SystemExit(Exception(cmd))
        ...
        if function != None:
            for param in ctx.parametro():
                if self.searchSymbolTable(param.ID_L().getText()) == None:
                    valorInicial = self.atribuirValorInicialParam(
                        param.TIPO().getText())
                    self.jasmin.passingParameters(param.TIPO().getText())
                else:
                    raise SystemExit(Exception("Parâmetro já declarado."))
                if len(self.tabelaDeSimbolos.keys()) == 0:
                    _id = 0
                    self.tabelaDeSimbolos[_id] = [param.ID_L().getText(
                    ), valorInicial, param.TIPO().getText(), self.escopo, "param"]
                else:
                    _id = max(self.tabelaDeSimbolos.keys()) + 1
                    self.tabelaDeSimbolos[_id] = [param.ID_L().getText(
                    ), valorInicial, param.TIPO().getText(), self.escopo, "param"]
                self.tabelaNameFunctions[function][EXPECTED_TYPES].append(
                    param.TIPO().getText())
            ...
            self.tabelaNameFunctions[function][LAST_LOCAL_USED] += 1
            self.jasmin.createScanner(
                self.tabelaNameFunctions[function][LAST_LOCAL_USED])
    ...
    def enterRetorno(self, ctx: jauanParser.RetornoContext):
        if not self.inFunction(ctx):
            raise SystemExit(Exception("'return' fora de função."))
    ...
    def exitRetorno(self, ctx: jauanParser.RetornoContext):
        ...
        if _type != self.jasmin.function_return_type:
            ...
            raise SystemExit(Exception(ret))
        self.jasmin.returnType()
    ...
    def exitComando_atribuicao(self, ctx: jauanParser.Comando_atribuicaoContext):
        ...
        var = ctx.id_(0).id
        child = [i for i in ctx.children if not isinstance(
            i, TerminalNodeImpl)]
        child = child[1]
        ...
        if self.tabelaDeSimbolos[var][VAR_OR_CONST] in ['var', 'param']:
            if ctx.id_(0).type == child.type:
                ...
                cmd = "A variável " + \
                    str(ctx.id_(0).name) + \
                    " não é do mesmo tipo do valor atribuído"
                raise SystemExit(Exception(cmd))
        else:
            cmd = str(ctx.getChild(0).name) + \
                " é uma constante. Era esperado uma variável"
            raise SystemExit(Exception(cmd))
    ...
    def exitMultDiv(self, ctx: jauanParser.MultDivContext):
        _type = 'float' if 'float' in [ctx.op_algebrico(
            0).type, ctx.op_algebrico(1).type] else 'int'
        if _type == 'float':
        ...
        if ctx.op.text == '*':
            ...
            raise SystemExit(Exception("Erro: Tipos incompativeis."))
        if hasattr(ctx, 'inh') and ctx.inh == 'print':
    ...
    def enterWhile(self, ctx: jauanParser.WhileContext):
        ...
        if ctx.exprRelacionalBinaria():
            ...
            ctx.inst_funcao().inh='while'
            self.jasmin.constructIf('!=', 'zero')
        elif ctx.exprRelacionalUnaria():
            ctx.exprRelacionalUnaria().inh='while'
            self.jasmin.constructIf('==', 'zero')
    ...
    def enterBreak(self, ctx: jauanParser.BreakContext):
        if not self.inLoop(ctx):
            raise SystemExit(Exception("'break' fora de loop."))
    ...
    def exitInst_funcao(self, ctx: jauanParser.Inst_funcaoContext):
        ...
        self.jasmin.invokeFunction(
            ctx.id_().name, ctx.args_real().type, ctx.type)
        if hasattr(ctx, 'inh') and ctx.inh == 'print':
            if ctx.type != 'void':
                if ctx.type == 'bool':
                ...
                self.jasmin.StringBuilderAppend('str')
                # raise SystemExit(Exception("Não é possível imprimir uma função void.")
        ...
        if isinstance(ctx.parentCtx, jauanParser.Inst_funcaoContext):
            ...
            if function_id != None:
                numero_parametros=len(
                    self.tabelaNameFunctions[function_id][EXPECTED_TYPES])
                if ctx.children:
                    args_passer=[
                        i for i in ctx.children if not isinstance(i, TerminalNodeImpl)]
                else:
                    args_passer=[]
                if numero_parametros != len(args_passer):
                    cmd="A função '" + name_function + "' espera " + \
                        str(len(self.tabelaNameFunctions[function_id][EXPECTED_TYPES])) + \
                        " parâmetros, mas foram recebidos " + \
                        str(len(ctx.children))
                    raise SystemExit(Exception(cmd))
                else:
                    for i in zip(args_passer, self.tabelaNameFunctions[function_id][EXPECTED_TYPES]):
                        if i[0].type != i[1]:
                            cmd="A função '" + name_function + "' espera o tipo '" + \
                                i[1] + "' mas foi recebido '" + i[0].type + "'"
                            raise SystemExit(Exception(cmd))
            else:
    ...
    def structElse(self, ctx):
        ...
        parent=ctx
        while not isinstance(parent.parentCtx, jauanParser.IfElseContext):
            parent=parent.parentCtx
        if isinstance(parent.parentCtx.else_(), jauanParser.ElseContext):
            parent.parentCtx.else_().lb_else=lb_else
            parent.parentCtx.else_().lb_end=lb_end
    ...
    def exitExprRelacionalBinaria(self, ctx: jauanParser.ExprRelacionalBinariaContext):
        ctx.type='bool'
        _type='int' if ctx.op_relacional(0).type in ['int', 'bool'] and ctx.op_relacional(
            1).type in ['int', 'bool'] else 'float'
        if ctx.op_relacional(0).type in ['int', 'float', 'bool'] and ctx.op_relacional(1).type in ['int', 'float', 'bool']:
            ...
            cmd="Tipos das variaveis '" + ctx.op_relacional(0).getText(
            ) + "' e '" + ctx.op_relacional(1).getText() + "' incompativeis."
            raise SystemExit(Exception(cmd))
    ...
    def testeLogico(self, operador=None, _type=None):
        ...
        _id=self.searchNameFunction(self.escopo)
        self.tabelaNameFunctions[_id][LAST_LOCAL_USED] += 1
        self.jasmin.createNewTemp(
            self.tabelaNameFunctions[_id][LAST_LOCAL_USED], 'int')
    ...
    def enterTerm(self, ctx: jauanParser.TermContext):
        if hasattr(ctx, 'inh'):
            [i for i in ctx.children if not isinstance(
                i, TerminalNodeImpl)][0].inh=ctx.inh
    ...
    def exitTerm(self, ctx: jauanParser.TermContext):
        if ctx.id_():
            if ctx.id_().type == 'bool':
                ...
                cmd="Erro: era esperado da variável '" + \
                    ctx.id_().name + "' o tipo 'bool', mas foi recebido '" + ctx.id_().type + "'"
                raise SystemExit(Exception(cmd))
        elif ctx.value():
